...
host = "144.76.119.111"  # Address of elastic search node
...
def search_query(search):
    url = "http://46.4.40.237:1478/search?q=" + search.query
    if search.language is not None:
        url += "&lang=" + search.language
    ...
    return {
        "items": response.json()['searchItemList'],
        "searchTime": response.json()['searchTime']
    }